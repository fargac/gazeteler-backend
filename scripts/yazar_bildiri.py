"""
Yazar Bildirim Sistemi v4.0 (Enterprise Scraper Edition)
SonarQube S3776 (Cognitive Complexity) ve Boilerplate optimizasyonları yapıldı.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 🔥 YENİ: ORTAK YARDIMCI (HELPER) FONKSİYONLAR
# (Boilerplate kodu %40 oranında azaltır)
# ─────────────────────────────────────────────────────────────────────────────

def get_soup(url: str, is_xml: bool = False) -> BeautifulSoup:
    """URL'yi çeker ve doğrudan parse edilmiş Soup objesini döner."""
    if url not in FETCH_CACHE:
        r = secure_session.get(url, timeout=15)
        r.raise_for_status()
        FETCH_CACHE[url] = BeautifulSoup(r.content if is_xml else r.text, "xml" if is_xml else "html.parser")
...
def send_error_email(author_name: str, error_message: str):
    ...
    if not msg_user or not msg_pass: return
...
# ─────────────────────────────────────────────────────────────────────────────
# ÖZEL SCRAPER FONKSİYONLARI (Flat Design / Guard Clauses uygulanmıştır)
# ─────────────────────────────────────────────────────────────────────────────
...
def find_from_rss(author: dict) -> Optional[dict]:
    ...
    for item in soup.find_all("item"):
        ...
        if not title_tag or author["match"].lower() not in title_tag.text.lower(): continue
        ...
        if pub_tag:
            try:
                ...
                if parsedate_to_datetime(pub_tag.text).astimezone(TR_TZ).date() != today: continue
...
def find_from_hurriyet(author: dict) -> Optional[dict]:
    ...
    for box in soup.select("a.author-box"):
        ...
        if not name_tag or author[KEY_NAME].lower() not in name_tag.text.lower(): continue
...
def find_from_karar(author: dict) -> Optional[dict]:
    ...
    for article in soup.select("section.author-article article.item, article.item.box-shadow"):
        ...
        if time_tag and time_tag.get("datetime", "")[:10] != today_str: continue
...
def find_from_t24(author: dict) -> Optional[dict]:
    ...
    for a in soup.select("section.aramakartalanimobil a[href]"):
        ...
        if date_tag and today_day_str not in date_tag.text: continue
...
def find_from_cumhuriyet(author: dict) -> Optional[dict]:
    ...
    for a in soup.select("div#articles-container a[href]"):
        ...
        if time_tag:
            ...
            if today_str not in date_text and today_day_str not in date_text: continue
...
def find_from_mahfi(author: dict) -> Optional[dict]:
    ...
    for article in soup.select("article.post"):
        ...
        if date_tag and today_year_str not in (date_tag.get("datetime", "") or date_tag.get("title", "") or date_tag.text): continue
...
def find_from_star(author: dict) -> Optional[dict]:
    ...
    for li in soup.select("ul.main li"):
        ...
        if date_div and today_day_str not in date_div.text: continue
...
def find_from_nefes(author: dict) -> Optional[dict]:
    ...
    for article in soup.select("section.author-posts article.article-card"):
        time_tag = article.find("time")
        if time_tag and today_day_str not in time_tag.text: continue
...
def find_from_habervakti(author: dict) -> Optional[dict]:
    ...
    for card in soup.select("div.card"):
        ...
        if date_div and "Bugün" not in date_div.text and today_day_str not in date_div.text: continue
...
def find_from_nihal(author: dict) -> Optional[dict]:
    ...
    for li in soup.select("div#infinite-data li, ul.articles-list li"):
        ...
        if date_p and today_str not in date_p.text and today_day_str not in date_p.text: continue
        ...
        if date_span:
            try:
                ...
                if len(parts) == 3:
                    ...
                    if date_type(int(parts[2]), TR_MONTHS.get(parts[1], 0), int(parts[0])) != today: continue
...
def process_author(author: dict, sent_ref):
    ...
    try:
        ...
        if not finder: return log.warning(f"  → Finder yok: {author[KEY_SOURCE]}")

        article = finder(author)
        if not article: return log.info(f"  → {author[KEY_NAME]}: Bugün makale yok.")
        ...
        try:
            doc_ref.create({"url": article[KEY_URL], "authorId": author[KEY_ID], "authorName": author[KEY_NAME], "sentAt": firestore.SERVER_TIMESTAMP})
...
def main():
    ...
    if not cred_json: raise EnvironmentError("FIREBASE_CREDENTIALS environment variable eksik!")
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for f in futures:
            try: f.result() 
            except Exception as e: log.error(f"Kritik Thread Hatası: {e}")
